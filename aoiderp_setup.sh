...
echo "STEP 5:Config RetroPIE"
echo "1.config autostart"
sed -i '/zpod/'d /etc/rc.local
sed -i '/play_pitft/'d /etc/rc.local
sed -i '/^emulationstation/icd \/home\/pi\/zpod' /opt/retropie/configs/all/autostart.sh
sed -i '/^emulationstation/isudo .\/play_pitft' /opt/retropie/configs/all/autostart.sh
echo "2.config themes"
...
cp /home/pi/zpod/resources/SourceHanSans-Regular.ttc /etc/emulationstation/themes/carbon/art/Cabin-Bold.ttf
if ! grep -q AOIDE "/etc/emulationstation/es_systems.cfg"; then
  sed -i '/^<\/systemList>/i  <system>' /etc/emulationstation/es_systems.cfg
  sed -i '/^<\/systemList>/i    <name>AOIDE<\/name>' /etc/emulationstation/es_systems.cfg
  sed -i '/^<\/systemList>/i    <fullname>AOIDE Player<\/fullname>' /etc/emulationstation/es_systems.cfg
  sed -i '/^<\/systemList>/i    <path>\/home\/pi\/zpod<\/path>' /etc/emulationstation/es_systems.cfg
  sed -i '/^<\/systemList>/i    <extension>.sh<\/extension>' /etc/emulationstation/es_systems.cfg
  sed -i '/^<\/systemList>/i    <command>%ROM%<\/command>' /etc/emulationstation/es_systems.cfg
  sed -i '/^<\/systemList>/i    <platform>AOIDE<\/platform>' /etc/emulationstation/es_systems.cfg
