...
sed -i "$linenumfinala rm aoide_volumio_4.9.51.tar.gz" scripts/raspberryconfig.sh
sed -i "$linenumfinala tar xf aoide_volumio_4.9.51.tar.gz" scripts/raspberryconfig.sh
sed -i "$linenumfinala echo 'Extracting Aoide-DACs Modules'" scripts/raspberryconfig.sh
sed -i "$linenumfinala wget https://github.com/howardqiao/volumio2-aoide-drivers/raw/master/aoide_volumio_4.9.51.tar.gz" scripts/raspberryconfig.sh
